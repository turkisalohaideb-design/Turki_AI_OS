...
const LaunchSchema = z.object({ id: z.string(), options: z.any().optional() })
...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch(`${DAEMON}/api/agents/launch`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(parsed.data) })
